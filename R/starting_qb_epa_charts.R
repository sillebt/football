...
starting_qb_epa <- function(szn) {
  team_info <- load_teams() %>%
    select(team = team_abbr, team_conf, team_color)
  ...
  ragg::agg_png("magick/Base.png", width = 24, height = 12, units = "in", res = 300)
  base
  dev.off()

  base_img <- magick::image_read("magick/Base.png")

  base_raster <- grid::rasterGrob(base_img, width = unit(1, "npc"),
                                  height = unit(1, "npc"))

  plot_with_personal <- add_logo(
    plot_path = "magick/Base.png",
  ...
  magick::image_write(plot_with_nfl, "figures/NFL_QB_Cum_EPA.png")
}
