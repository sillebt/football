nfl_weekly_efficiency <- function(franchise) {
  ...
  week_num <- max(nfl_pbp$week)

  c <- efficiency_over_time(model_dataset, current_season = szn, current_week = week_num, team_name = franchise)

  ggsave(plot = c, glue('figures/efficiency_trend_{franchise}_{week_num}.png'))
